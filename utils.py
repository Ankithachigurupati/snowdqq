...
def html(click):
    if click:
        right_popup_visible = True
    else:
        right_popup_visible = False
    
    right_popup_content = """
        <div style="position: fixed; z-index:9999; top: 0; right: 0; width:  1000px; height: 100%;background-color: white; box-shadow: -5px 0 5px rgba(0, 0, 0, 0.2);">
        <br>
        <br>
        <h5>Expectation Details</h5>
        <hr>
        <h5>expect_column_values_to_not_be_in_set</h5>
        <hr>
        <label>Description</label>
        <p>except column values to not be in set</p>
        <hr>
        <label> Tags</label>
        <p>[ "["core expectation", "column map expectation"]" ]<p>
        <hr>
        <p>Backend support()</P>
        <hr>
        <button style="position: absolute; bottom: 20px; right: 20px;" onclick="closePopup()">Cancel</button>
        </div>
